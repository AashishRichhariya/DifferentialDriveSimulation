...
//following variables are required for MDFS
   ...
   int tree_id;//required for MDFS
    ...
    bool isBT;//is backtrack point
    bool isUEV;//is UEV point
   ...
   nd():tot(0),blacks(0),whites(0),tot_x(0),tot_y(0){
    ...
    steps = bot_presence.first = isBoundaryCell = visited = visit_cost = checked =  observed = mark_visited = isBT = isUEV = manhattan_dist = 0;
  }
  void emptyCell(){
    tot = blacks = whites = tot_x = tot_y = steps = bot_presence.first = visited = isBoundaryCell = visit_cost = checked = observed =  mark_visited = isBT = isUEV = manhattan_dist = 0;
